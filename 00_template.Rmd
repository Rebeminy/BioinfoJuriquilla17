...
```{r quest2}
result<-data.frame(unique(pharmacoData$concentration))
print(result)
dim(result)
```
...
```{r quest3}
ggplot(data=pharmacoData, aes(viability))+geom_histogram()+facet_wrap(~drug)
```
...
```{r ques4}
table=tapply(pharmacoData$viability, pharmacoData$drug, function(x){length(x[x>=0 & x<=100])})
sum(pharmacoData$viability >= 0 & pharmacoData$viability <=100)
data.frame(sort(table))
```
...
```{r quest3}
ggplot( pharmacoData, aes( viability, group=doseID, colour=doseID) ) +
    geom_histogram(fill="white", lwd=2, alpha=0.1) + xlim(0, 170) + facet_wrap(~doseID)
``` 
...
```{r tut2 ques1}
library(ggplot2)
ggplot(aes(x=auc_GDSC, y=auc_CCLE, colour=drug), data=subset(cellLinesSummary)) + geom_point() +
    xlab("GDSC AUC") +
    ylab("CCLE AUC") 
```
